...
class Compositor {
  composite(
    ...
    this.compositeItems(
      bounds,
      preset.preset_root.viewgroup_items as (Item & HasSize & HasPadding)[],
      preset.preset_root,
    );
    ...
    switch (item.internal_type) {
      case 'OverlapLayerModule': {
        item.viewgroup_items.forEach((item) => {
          const { size: itemSize, padding: itemPadding } = this.initItem(item);

          const totalItemSize: Size = {
            width: itemSize.width + itemPadding.left + padding.right,
            height: itemSize.height + itemPadding.top + itemPadding.bottom,
          };
