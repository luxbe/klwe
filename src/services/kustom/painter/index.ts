  ...
  drawItem(item: Item & HasBox, ctx: CanvasRenderingContext2D) {
    ...
    switch (item.internal_type) {
      case 'ShapeModule': {
        ...
        switch (item.shape_type) {
          ...
          case 'TRIANGLE': {
            ...
            ctx.moveTo(c, 0);
            ctx.lineTo(item.shape_width ?? 20, item.shape_height ?? 20);
      ...
      case 'TextModule': {
        ...
        const fontSize =
          !item.text_size_type || item.text_size_type === 'FIXED_WIDTH'
            ? item.text_size ?? 20
            : 0;
        ...
        for (let i = 0; i < lines.length; i++) {
          ctx.fillText(lines[i], position.x, position.y + fontSize * (i + 1));
        }
      }
    }
  }
